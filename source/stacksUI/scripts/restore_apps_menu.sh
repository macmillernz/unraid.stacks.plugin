#!/bin/bash
# Reverses patch_apps_menu.sh using the one-time backup it made.
set -e

PLUGIN_DIR="/usr/local/emhttp/plugins/stacksUI"
BACKUP_DIR="$PLUGIN_DIR/.apps-menu-backup"
CA_DIR="/usr/local/emhttp/plugins/community.applications"

if [ ! -d "$BACKUP_DIR" ]; then
  echo "stacksUI: no Apps-tab backup found, nothing to restore" >&2
  exit 0
fi

for BACKUP_FILE in "$BACKUP_DIR"/*.page; do
  [ -e "$BACKUP_FILE" ] || continue
  TARGET="$CA_DIR/$(basename "$BACKUP_FILE")"
  if [ -f "$TARGET" ]; then
    cp "$BACKUP_FILE" "$TARGET"
    echo "stacksUI: restored $TARGET"
  fi
done
