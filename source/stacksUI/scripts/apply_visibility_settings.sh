#!/bin/bash
# Single entry point that reads settings.json and applies both tab-
# visibility toggles (hideDocker/hideApps). Run at plugin install/upgrade,
# at every boot (via /boot/config/go - see stacksUI.plg), and immediately
# whenever the user saves the Settings page (see
...
# Defaults here (true/true) match stacksUI_settings()'s own PHP defaults -
# kept in sync manually since this reads the same file from a different
# language; if you change one, change the other.
read_flag() {
  php -r '
