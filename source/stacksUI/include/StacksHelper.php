...
// Stack names must start with a letter/digit; "." is allowed after the
// first character (some real-world compose project names use a dotted
// form, e.g. "changedetection.io") but "/" never is, so no matched
// string can ever contain a ".." path segment.
...
// hideDocker/hideApps control which native Unraid tabs are visible
// (Docker and Community Applications' Apps tab respectively) - see
// stacksUI_apply_visibility_settings() for how these are actually
// enforced. Both default to true, matching this plugin's original
// behavior so upgrading doesn't change anything for existing installs
...
// and the Community Applications "Apps" tab, based on whatever
// stacksUI_settings() currently returns. Best-effort: a failure here
// shouldn't block saving the settings themselves (mirrors the "|| true"
// pattern already used for these same scripts at plugin install time in
// stacksUI.plg).
...
// stacksUI_write_stack() itself has no duplicate-name guard (it's used by
// both "create" and "update" - "update" legitimately writes over an
// existing stack) - ajax.php's "create" case calls this first to make
// sure a brand new stack never silently overwrites one that already
// exists.
